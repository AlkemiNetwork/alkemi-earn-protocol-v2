    ...
    ExponentialNoError
{
    ...
    function initializer(
        ...
        if (initializationDone == false) {
            ...
            owner = _owner;
            alkemiEarnVerified = AlkemiVerifiedInterface(_alkemiEarnVerified);
            alkemiEarnPublic = AlkemiPublicInterface(_alkemiEarnPublic);
    ...
    function claimAlk(
        ...
        require(allMarketsIndex[isVerified][market], Error.MARKET_NOT_SUPPORTED);
        ...
        returns (Exp[] memory, Exp memory)
    {
        ...
        uint256 currentMarketTotalSupply;
        uint256 currentMarketTotalBorrows;
        ...
        for (uint256 i = 0; i < allMarkets[true].length; i++) {
            ...
            currentMarketTotalSupply = mul_(
                alkemiEarnVerified.getMarketTotalSupplyVerified(currentMarket),
                alkemiEarnVerified.assetPrices(currentMarket)
            );
            currentMarketTotalBorrows = mul_(
                alkemiEarnVerified.getMarketTotalBorrowVerified(currentMarket),
                alkemiEarnVerified.assetPrices(currentMarket)
            );
        ...
        for (uint256 j = 0; j < allMarkets[false].length; j++) {
            ...
            currentMarketTotalSupply = mul_(
               alkemiEarnPublic.getMarketTotalSupplyPublic(currentMarket),
               alkemiEarnVerified.assetPrices(currentMarket)
            );
            currentMarketTotalBorrows = mul_(
                alkemiEarnPublic.getMarketTotalBorrowPublic(currentMarket),
                alkemiEarnVerified.assetPrices(currentMarket)
            );
            currentMarketTotalLiquidity = Exp({
                mantissa: add_(
    ...
    function updateAlkSupplyIndex(address market, bool isVerified) public {
        ...
        if (deltaBlocks > 0 && marketSpeed > 0) {
            ...
            alkSupplyState[isVerified][market] = MarketState({
                index: safe224(index.mantissa, Error.NEW_INDEX_EXCEEDS_224_BITS),
                block: safe32(blockNumber, Error.BLOCK_NUMBER_EXCEEDS_32_BITS)
            });
        } else if (deltaBlocks > 0) {
            supplyState.block = safe32(
                blockNumber,
                Error.BLOCK_NUMBER_EXCEEDS_32_BITS
            );
        }
    }
    ...
    function updateAlkBorrowIndex(address market, bool isVerified) public {
        ...
        if (deltaBlocks > 0 && marketSpeed > 0) {
            ...
            alkBorrowState[isVerified][market] = MarketState({
                index: safe224(index.mantissa, Error.INTEGER_OVERFLOW),
                block: safe32(blockNumber, Error.INTEGER_OVERFLOW)
            });
        } else if (deltaBlocks > 0) {
    ...
    {
        if(isVerified){
            return alkemiEarnVerified.getMarketTotalSupplyVerified(market);
        }
        else{
            return alkemiEarnPublic.getMarketTotalSupplyPublic(market);
        }
    }
    ...
    {
         if(isVerified){
            return alkemiEarnVerified.getMarketTotalBorrowVerified(market);
        }
        else{
            return alkemiEarnPublic.getMarketTotalBorrowPublic(market);
        }
    }
    ...
    function transferOwnership(address _newOwner) external onlyOwner {
        require(_newOwner != owner, Error.UNAUTHORIZED);
        newOwner = _newOwner;
    }
    ...
    function addMarket(address market, bool isVerified) external onlyOwner {
        require(!allMarketsIndex[isVerified][market], Error.SAME_ADDRESS);
        require(
    ...
    function setAlkAddress(address _alkAddress) external onlyOwner {
        require(alkAddress != _alkAddress, Error.SAME_ADDRESS);
        require(_alkAddress != address(0), Error.ADDRESS_CANNOT_BE_0X00);
        alkAddress = _alkAddress;
    }
    ...
    function getSupplyAlkRewards(
        ...
        if (
            ...
            supplyState = MarketState({
                index: safe224(index.mantissa, Error.NEW_INDEX_EXCEEDS_224_BITS),
                block: safe32(getBlockNumber(), Error.BLOCK_NUMBER_EXCEEDS_32_BITS)
            });
