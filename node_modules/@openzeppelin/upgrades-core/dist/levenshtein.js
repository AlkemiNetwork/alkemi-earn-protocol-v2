...
function levenshtein(a, b, match) {
...
function buildMatrix(a, b, eq) {
    const matrix = new Array(a.length + 1);
    const insertionCost = (i, j) => (j > a.length ? 0 : INSERTION_COST);
    const substitutionCost = (i, j) => (eq(a[i - 1], b[j - 1]) ? 0 : SUBSTITUTION_COST);
    const deletionCost = () => DELETION_COST;
    ...
    for (let i = 1; i <= a.length; i++) {
        for (let j = 1; j <= b.length; j++) {
            matrix[i][j] = Math.min(matrix[i - 1][j - 1] + substitutionCost(i, j), matrix[i][j - 1] + insertionCost(i, j), matrix[i - 1][j] + deletionCost(i, j));
        }
    }
    return matrix;
}
// Walks an edit distance matrix, returning the sequence of operations performed
function walkMatrix(matrix, a, b, match) {
