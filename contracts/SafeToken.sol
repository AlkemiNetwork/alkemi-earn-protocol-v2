...
contract SafeToken {
    ...
    function checkTransferIn(
        ...
        require(token.allowance(from, address(this)) >= amount, Error.TOKEN_INSUFFICIENT_ALLOWANCE);

        require(token.balanceOf(from) >= amount, Error.TOKEN_INSUFFICIENT_BALANCE); 
    }
    ...
    function doTransferIn(
        ...
        require(result, Error.TOKEN_TRANSFER_FAILED); 
     ...
     */
    function getCash(address asset) internal returns (uint256) {
        EIP20Interface token = EIP20Interface(asset);
    ...
    function doTransferOut(
        ...
        require(result, Error.TRANSFER_OUT_FAILED); 
        return true;
    }
